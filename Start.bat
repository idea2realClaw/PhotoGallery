...
REM Most-suitable existing ARM64 venv on this machine (has all three):
...
REM 若依赖未安装则自动安装（app.py 还需要 numpy，可选 pyftpdlib 用于 FTP 共享）
"%PYTHON%" -c "import flask, PIL, numpy" 2>nul
if errorlevel 1 (
    echo [安装依赖 flask + Pillow + numpy + pyftpdlib ...]
    "%PYTHON%" -m pip install flask pillow numpy pyftpdlib
)
